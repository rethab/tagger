...
oneTrack = let artist = Artist "Agrypnie" [album]
               album = Album "Exit" [track] Nothing Nothing
           in do 
                 dir <- getDataDir 
                 let track = Track "1-Mauern.mp3" (dir </> "Agrypnie/Exit")
                             (Just "Mauern") (Just 1)
                 writeTags [artist]
...
albumOnly = let artist = Artist "Agrypnie" [album]
                album = Album "Exit" [track] (Just 2008) (Just "Black Metal")
            in do dir <- getDataDir 
...
writeBoth = let artist = Artist "Agrypnie" [album]
                album = Album "Exit" [track] (Just 2008) (Just "Black Metal")
            in do dir <- getDataDir
                  track = Track "3-Zorn.mp3" (dir </> "Agrypnie/Exit")
                                (Just "Zorn") (Just 3)
                  writeTags [artist]
                  artists <- listArtists dir
                  let agr = head $ filter (\a -> artName a == "Agrypnie") artists
